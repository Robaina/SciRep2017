...
%     data, in the form (1/CV(D))*||V-D||^2, where CV=std(D)/mean(D) 
...
%     active in the final model (i.e. flux value above eps)
...
%     Options.eps: scalar indicating the threshold value to consider a reaction active,
%     i.e. a reaction i with flux value vi is active if vi>=eps
...
%    Sol.ActRxn: set of active (i.e. |Vi|>eps) reactions
...
if ~isfield(Options,'FeasTol'),
    ...
    if isfield(Options,'Carb2Oxy') && Options.Carb2Oxy=='T',   
     ...
     vecsense=[repmat('=',Mets,1);repmat('=',(NIrr_D+NRev_D),1);repmat('<',NRev,1);repmat('=',NRev_D,1);repmat('<',NRev,1);repmat('>',NRev,1);repmat('>',NRev,1);'<';'>'];
     b=[zeros(Mets,1);DIrr;DFor;Revub;zeros(NRev_D,1);zeros(NRev,1);Revlb;zeros(NRev+2,1)];
     Amat=[A0;A1;A2;A3;A4;A5;A6;A7;A8;A9];
